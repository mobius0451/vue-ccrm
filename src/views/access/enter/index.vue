<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="入校类型" prop="outType">
        <el-select v-model="queryParams.enterType" placeholder="选择入校类型" clearable>
          <el-option
      ...
      <el-form-item label="审核状态" prop="status" v-hasPermi="['access:enter:check']">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          icon="el-icon-plus"
          size="mini"
          @click="handleNoCheckEnterAdd"
          v-hasPermi="['access:enter:noCheck']"
        >无审入校</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['access:enter:remove']"
        >删除</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="enterList" @selection-change="handleSelectionChange">
      ...
      <el-table-column type="expand"  v-hasPermi="['access:enter:noCheck']">
        <template slot-scope="props">
          <div>
            ...
            <el-form inline class="demo-table-expand" style="margin-top: 10px">
              ...
              <el-form-item label="启程地:">
                <span>{{ props.row.departurePlace }}</span>
              </el-form-item>
              <el-form-item label="入校原因:">
                <span>{{ props.row.enterReason  }}</span>
              </el-form-item>
            </el-form>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="序号" align="center" prop="enterId" width="100" />
      ...
      <el-table-column label="入校类型" align="center" prop="enterType" width="200">
        <template slot-scope="scope">
          <my-tag :options="enterType" :value="scope.row.enterType"/>
        </template>
      </el-table-column>
    ...
    <el-dialog title="无审入校" :visible.sync="open">
...
<script>
import { listEnter,  delEnter, addEnter } from "@/api/access/enter";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listEnter(this.queryParams, this.dateRange).then(response => {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.enterId)
      this.single = selection.length!=1
      this.multiple = !selection.length
    },
    /** 申请入校操作 */
    handleOutAdd: function() {
      this.$router.push("/access/detail/enter/" + 0);
    },
    /** 审批申请操作 */
    handleOutCheck: function(row) {
      const enterId = row.enterId;
      this.$router.push("/access/check/enter/" + enterId);
    },
    /** 申请详情操作 */
    handleOutDetail: function(row) {
      const enterId = row.enterId;
      this.$router.push("/access/detail/enter/" + enterId);
    },
    /** 删除按钮操作 */
    handleDelete(row) {
      const enterIds = row.enterId || this.ids
      this.$modal.confirm('是否确认删除公告编号为"' + enterIds + '"的数据项？').then(function() {
        return delEnter(enterIds);
      }).then(() => {
        this.getList();
