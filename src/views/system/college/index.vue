<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch">
      <el-form-item label="学院班级名称" prop="collegeName">
        <el-input
          v-model="queryParams.collegeName"
          placeholder="请输入学院班级名称"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="学院班级状态" clearable>
          <el-option
    ...
    >
      <el-table-column prop="collegeName" label="学院班级名称" width="260"></el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="学院班级名称" prop="collegeName">
              <el-input v-model="form.collegeName" placeholder="请输入学院班级名称" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
export default {
  ...
  methods: {
      ...
      getCollege(row.collegeId).then(response => {
        ...
        listCollegeExcludeChild(row.collegeId).then(response => {
          ...
          if (this.collegeOptions.length == 0) {
            const noResultsOptions = { collegeId: this.form.parentId, collegeName: this.form.parentName, children: [] };
            this.collegeOptions.push(noResultsOptions);
          }
        });
      });
    },
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      this.$modal.confirm('是否确认删除名称为"' + row.collegeName + '"的数据项？').then(function() {
